...
def extract(logfile, rootDir, filename):
    global writeinrows
    ...
    # seek for bonder info from file path
    link = re.findall(r'(?<=TCB)\d(?=OpLog)', filename)
    # print(link)
    die_drop_data.insert(loc=11, column='Bonder', value=link[0])
        ...
        if os.path.isfile(pathname):
            ...
            if oplog:
                ...
                if filetime > comparetime.timestamp():
                    ...
                    # print(filetime)
                    extract(pathname, rootDir, filename)
        else:
            listDir(pathname)
